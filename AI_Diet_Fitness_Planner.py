...
def save_user(data, bmi, bmr, tdee, target):
    conn = sqlite3.connect(DB_FILE)
    conn.execute("""
        INSERT INTO users
        (name, age, gender, weight, height, goal, activity, bmi, bmr, tdee, target)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data["name"], data["age"], data["gender"], data["weight"],
        data["height"], data["goal"], data["activity"],
        float(bmi), float(bmr), float(tdee), float(target)
    ))
    conn.commit()
    conn.close()
...
def get_users():
    conn = sqlite3.connect(DB_FILE)
    result = pd.read_sql_query("SELECT * FROM users ORDER BY id DESC", conn)
    conn.close()
    return result
...
page = st.radio(
    "App Section",
    ["Planner", "Data Analysis", "User History"],
    horizontal=True,
...
if page == "Data Analysis":
    ...
    st.caption("Analysis of the diet_dataset.csv dataset.")

    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Total Food Items", len(df))
    col2.metric("Average Calories", f"{df['Calories'].mean():.1f}")
    col3.metric("Average Protein", f"{df['Protein'].mean():.1f} g")
    col4.metric("Categories", df["Category"].nunique())

    st.markdown("### 📋 Dataset")
    st.dataframe(df, use_container_width=True)

    st.markdown("### 🔥 Average Calories by Goal")
    goal_analysis = df.groupby("Goal")["Calories"].mean().round(1)
    st.bar_chart(goal_analysis)

    st.markdown("### 💪 Average Protein by Category")
    protein_analysis = df.groupby("Category")["Protein"].mean().round(1)
    st.bar_chart(protein_analysis)

    st.markdown("### 🍽️ Food-wise Calories & Protein")
    analysis_df = df[["Food_Item", "Calories", "Protein", "Category", "Goal"]].copy()
    st.dataframe(analysis_df.sort_values("Calories", ascending=False), use_container_width=True)
...
if page == "User History":
    ...
    if history.empty:
        st.info("No user records have been saved yet. Generate a plan from the Planner section first.")
    else:
        c1, c2, c3 = st.columns(3)
        c1.metric("Total Users", len(history))
        c2.metric("Weight Loss Plans", int((history["goal"] == "Weight Loss").sum()))
        c3.metric("Weight Gain Plans", int((history["goal"] == "Weight Gain").sum()))

        st.dataframe(history, use_container_width=True)

        st.markdown("### 🎯 Goal Distribution")
        st.bar_chart(history["goal"].value_counts())
...
# ------------------------- STEP 3: RESULT (DASHBOARD) -------------------------
    ...
    # Prevent duplicate database records during Streamlit reruns
    ...
    if not st.session_state.saved_record:
        save_user(data, bmi, bmr, tdee, target)
